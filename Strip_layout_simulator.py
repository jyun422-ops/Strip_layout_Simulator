...
# ============================================================
# [2] DXF 읽기: 원형도 검증 알고리즘이 적용된 블록/스플라인/원 재귀 분석
# ============================================================
def extract_entities_recursive(entity_container, unit_factor, flatten_tol, candidates):
    """BLOCK/INSERT 및 CIRCLE, SPLINE 등을 재귀적으로 완벽히 추출합니다."""
    for entity in entity_container:
        if entity.dxftype() == 'INSERT':
            try:
                for virt_entity in entity.virtual_entities():
                    extract_entities_recursive([virt_entity], unit_factor, flatten_tol, candidates)
            except Exception:
                continue
        elif entity.dxftype() in ['LWPOLYLINE', 'POLYLINE', 'CIRCLE', 'ARC', 'ELLIPSE', 'SPLINE']:
            try:
...
def read_part_with_holes(msp, unit_factor):
    ...
    # 도면의 모든 객체(블록 내부 포함)를 재귀적으로 추출
    extract_entities_recursive(msp, unit_factor, flatten_tol, candidates)
    ...
    # ⭐ 핵심 수정: 원형도(Circularity) 검증을 통해 참조 원(치수, 피치원)이 외곽선으로 잡히는 버그 완벽 차단
    for poly in candidates:
        ...
        if circularity > 0.95:  # 95% 이상 완벽한 원
    ...
    for c in candidates:
        if c is outer:
            continue
        # 외곽선 내부에 포함되는 도형을 홀로 인식 (미세 오차 허용)
...
# ============================================================
# [4] 스트립 Layout도 렌더링 (캐리어 + Pilot 홀 포함)
# ============================================================
...
st.sidebar.header("🔧 3. 캐리어 & Pilot 설계")
...
pilot_dia = st.sidebar.number_input("Pilot 홀 지름 (mm)", value=4.0, step=0.5)

st.sidebar.header("🛠️ 4. Layout도 설계")
st_notch = st.sidebar.number_input("노칭 / Pilot 홀", value=1, step=1)
    ...
    with st.spinner('단위 변환, 내측 홀 인식, 안전 간격 적용 및 정밀 형상 맞춤(Nesting)을 분석 중입니다... (약 15초 소요)'):
        ...
        if part is None:
            ...
            st.markdown(f"좌측에서 입력하신 **총 {total_stations} 피치**, **캐리어 폭 {carrier_width}mm**, "
                        f"**Pilot 홀 ⌀{pilot_dia}mm** 조건을 반영한 실제 금형 내부 작업 구간 설계 도면입니다.")
